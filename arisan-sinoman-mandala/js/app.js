const rupiah = (num) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(num || 0);
...
function renderStats() {
  const income = state.cash.filter(x => x.type === 'in').reduce((sum, x) => sum + x.amount, 0);
  const expense = state.cash.filter(x => x.type === 'out').reduce((sum, x) => sum + x.amount, 0);
...
function renderMonths() {
  ...
  monthFilter.innerHTML = '<option value="all">Semua Bulan</option>' + months.map(month => `<option value="${month}">${month}</option>`).join('');
}

function renderCashTable() {
  const selected = monthFilter.value || 'all';
  const rows = state.cash.filter(item => selected === 'all' || item.month === selected);
  cashTable.innerHTML = rows.map(item => `
    <tr>
...
function renderSavings() {
  ...
  memberSavings.innerHTML = state.savings.map(item => `
...
function renderMinutes() {
  minutesList.innerHTML = state.minutes.map(item => `
    <div class="note-item">
...
function renderAnnouncements() {
  announcementList.innerHTML = state.announcements.map(item => `
...
loginForm.addEventListener('submit', (event) => {
  ...
  alert('Ini hanya tampilan demo. Untuk login aman gunakan API/serverless auth.');
});
